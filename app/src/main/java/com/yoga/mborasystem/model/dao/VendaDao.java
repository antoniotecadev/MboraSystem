...
public abstract class VendaDao {
    ...
    @Query("SELECT idpdvd, id, sum(preco_total) AS preco_total" +
    ...
    @Query("SELECT * FROM vendas WHERE (data_cria BETWEEN :dataInicio AND :dataFim) ORDER BY id ASC")
    public abstract Maybe<List<Venda>> getVendaSaft(String dataInicio, String dataFim);

    @Query("UPDATE vendas SET hashFR = :hashVenda WHERE id = :idvenda")
    ...
    public void insertVenda(List<String> vendas) {
        ...
        for (String vd : vendas) {
            ...
            venda.setDesconto(Integer.parseInt(vend[1]));
            venda.setPercentagemDesconto(Integer.parseInt(vend[2]));
            venda.setQuantidade(Integer.parseInt(vend[3]));
            venda.setValor_base(Integer.parseInt(vend[4]));
            venda.setReferenciaFactura(vend[5]);
            venda.setValor_iva(Integer.parseInt(vend[6]));
            venda.setPagamento(vend[7]);
            venda.setTotal_desconto(Integer.parseInt(vend[8]));
            venda.setTotal_venda(Integer.parseInt(vend[9]));
            venda.setDivida(Integer.parseInt(vend[10]));
            venda.setValor_pago(Integer.parseInt(vend[11]));
            venda.setEstado(Integer.parseInt(vend[12]));
            venda.setData_cria(vend[13]);
            venda.setData_elimina(vend[14]);
            venda.setIdclicant(Integer.parseInt(vend[15]));
            venda.setIdoperador(Integer.parseInt(vend[16]));
            venda.setData_cria_hora(vend[17]);
            venda.setHashFR(vend[18]);
            venda.setReferenciaNC(vend[19]);
            venda.setMotivoEmissaoNC(vend[20]);
            venda.setHashNC(vend[21]);
            venda.setData_cria_NC(vend[22]);
            venda.setData_cria_hora_NC(vend[23]);
            insert(venda);
        }
    }
