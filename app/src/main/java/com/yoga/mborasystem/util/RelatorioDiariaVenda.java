    ...
    private static void createPdfFile(boolean isGuardar, String path, String facturaPath, Activity activity, Context context, Cliente cliente, List<Venda> vendas, List<ProdutoVenda> produtoVendas, String data, Handler handler, View view) {
        ...
        try {
            ...
            for (Venda venda : vendas) {
                ...
                addNewLineWithLeftAndRight(document, String.valueOf(venda.getQuantidade()), formatPreco(String.valueOf(venda.getTotal_venda() - venda.getValor_iva())), font, font);
                addNewLineWithLeftAndRight(document, activity.getString(R.string.desconto) + "(" + venda.getPercentagemDesconto() + "%)", activity.getString(R.string.total), titleFont, titleFont);
                addNewLineWithLeftAndRight(document, formatPreco(String.valueOf(venda.getDesconto())), formatPreco(String.valueOf(venda.getTotal_desconto())), font, font);
                ...
                addNewLineWithLeftAndRight(document, formatPreco(String.valueOf(venda.getValor_pago())), formatPreco(String.valueOf(venda.getDivida())), font, font);
                ...
                addNewLineWithLeftAndRight(document, formatPreco(String.valueOf(venda.getDesconto() == 0 ? venda.getValor_base() : getValueWithDesconto(venda.getValor_base(), venda.getPercentagemDesconto()))), formatPreco(String.valueOf(venda.getDesconto() == 0 ? venda.getValor_iva() : getValueWithDesconto(venda.getValor_iva(), venda.getPercentagemDesconto()))), font, font);
            ...
            addLineSeparator(document);
            addNewItem(document, formatPreco(String.valueOf(totalVendas)), Element.ALIGN_RIGHT, font);
            addLineSeparator(document);
            addNewItem(document, activity.getString(R.string.geral), Element.ALIGN_CENTER, titleFont);
            addLineSeparator(document);
            addNewLineWithLeftAndRight(document, activity.getString(R.string.num_ven), activity.getString(R.string.tot_div), titleFont, titleFont);
            addNewLineWithLeftAndRight(document, String.valueOf(quantidadeVendas), formatPreco(String.valueOf(totalDividas)), font, font);
            ...
            addNewLineWithLeftAndRight(document, activity.getString(R.string.tot_des), activity.getString(R.string.total), titleFont, titleFont);
            addNewLineWithLeftAndRight(document, formatPreco(String.valueOf(totalDescontos)), formatPreco(String.valueOf(totalVendas - totalDescontos)), font, font);
            addLineSeparator(document);
            addLineSpace(document);
